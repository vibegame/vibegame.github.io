...
function checkSpeedOfFunction(n, func) {
...
let end = "СЛОН";
let n = 100;
console.log("Функция с быстрым перебором сработала " + n + " раз за " + checkSpeedOfFunction(n, findShortestWayToWord) + " секунд");
console.log("Функция с медленным перебором сработала " + n + " раз за " + checkSpeedOfFunction(n, findShortestWayToWord2) + " секунд");



function findShortestWayToWord2(dic, start, end) {
    function isNextWord(main_word, next_word) { // Функция на проверку 2 данных слов: являются ли они соседями?
        let number = 0;
        for(let i = 0;i<main_word.length;i++) {
                if(main_word[i] != next_word[i]) { // Если буква не равна соответствующей букве на этой позиции, то считаем сколько таких букв
